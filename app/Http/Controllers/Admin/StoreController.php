...
class StoreController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if($request->ward_id) {
                $ward = Ward::query()->where('id', $request->ward_id)->first();
            }
            ...
            $object->province_name = @$province->name ?? '';
            $object->district_id = $request->district_id;
            $object->district_name = @$district->name ?? '';
            $object->ward_id = $request->ward_id;
            $object->ward_name = @$ward->name ?? '';

            $object->province_slug = @$province->slug ?? '';
            $object->district_slug = @$district->slug ?? '';
            $object->ward_slug = @$ward->slug ?? '';
