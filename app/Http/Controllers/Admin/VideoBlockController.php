...
class VideoBlockController extends Controller
{
    ...
    public function searchData(Request $request)
    {
        ...
        return Datatables::of($objects)
            ...
            ->editColumn('link_youtube', function ($object) {
                return '<a href="' . $object->link_youtube . '" target="_blank">' . $object->link_youtube . '</a>';
            })
...
//                $result .= '<a href="' . route($this->route.'.delete', $object->id) . '" title="Xóa" class="btn btn-sm btn-danger confirm"><i class="fas fa-times"></i></a>';
            ...
            ->addIndexColumn()
            ->rawColumns(['image','action','link_youtube'])
            ->make(true);
    }
    ...
    public function store(Request $request)
    {
        $validate = Validator::make(
            ...
            [
                ...
                'image' => 'required|file|mimes:jpg,jpeg,png,svg,webp|max:3000',
            ]
        );
        $json = new stdClass();
        ...
        try {
            ...
            if($request->image) {
                FileHelper::uploadFileToCloudflare($request->image, $object->id, VideoBlock::class, 'image');
            }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            if($request->image) {
                if($object->image) {
                    FileHelper::deleteFileFromCloudflare($object->image, $object->id, \App\Model\Admin\VideoBlock::class, 'image');
                }
                FileHelper::uploadFileToCloudflare($request->image, $object->id, \App\Model\Admin\VideoBlock::class, 'image');
            }
    ...
    public function delete($id)
    {
        ...
        if($object->image) {
            FileHelper::deleteFileFromCloudflare($object->image, $object->id, \App\Model\Admin\Certification::class, 'image');
        }
        $object->delete();
        $message = array(
