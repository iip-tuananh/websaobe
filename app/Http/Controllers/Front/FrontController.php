...
use App\Model\Admin\Certification;
...
class FrontController extends Controller
{
    ...
    public function homePage() {
        ...
        //start
        $blockStory = Block::query()->with(['image', 'image_back'])->find(1);
        $messages = Partner::with(['image'])->latest()->get();
        $certifications = Certification::with(['image'])->latest()->get();
        $videos = VideoBlock::query()->with(['image'])->orderBy('id')->get();
        $partners = Channel::query()->with(['image'])
            ->get()->groupBy('type');
        ...
        return view('site.home', compact('about' , 'blogs', 'categories', 'videos',
            'banners', 'categoriesSpecial', 'messages', 'blockStory', 'certifications', 'partners'));
    }
    ...
    public function getProductList(Request $request, $slug = null) {
        ...
        return view('site.product_list', compact('products', 'banner', 'category', 'allCates'));
    }
    ...
    public function getProductDetail(Request $request, $slug = null) {
        $product = Product::query()->with(['category.banner', 'image', 'banner', 'galleries.image', 'types'])
            ->where('slug', $slug)->first();

        $productsLq = Product::query()->with(['image'])
            ->where('status', 1)
            ->whereNotIn('id', [$product->id])->latest()->take(2)->get();
        ...
        return view('site.product_detail', compact('product','productsLq', 'reviews', 'partners', 'blogs'));
    }
    ...
    public function getStores(Request $request, $proSlug = null, $distSlug = null)
    {
        $partners = Channel::query()->with(['image'])
            ->get()->groupBy('type');
        ...
        if(! $proSlug && ! $distSlug) {
            $stores = Store::query()
                ->select('province_id','province_name','province_slug','district_id','district_name', 'district_slug')
            ...
            $grouped = $stores
                ...
                ->map(function ($rows, $pid) {
                    ...
                    $provinceName = trim((string) (isset($first->province_name) ? $first->province_name : ''));
                    ...
                    $districts = $rows
                        ...
                        ->map(function ($r) {
                            return [
                                'id'   => (string) $r->district_id,
                                'name' => trim((string) $r->district_name),
                                'slug' => trim((string) $r->district_slug),
                            ];
                        })
                    ...
                    return [
                        ...
                        'province_slug' => $first->province_slug,
                        'districts'     => $districts,
                    ];
                })
            ...
            list($priority, $others) = $grouped->partition(function ($p) {
                return isset($p['province_id']) && $p['province_id'] === '1';
            });

            $result = $priority->concat($others)->values()->all();

            return view('site.store', compact('result', 'partners', 'banner'));
        } else {
            if(! $distSlug) {
                $stores = Store::query()
                    ->where('province_slug', $proSlug)
                    ->whereNotNull('province_id')
                    ->get();

                $result = $stores
                    ->groupBy('province_id')
                    ->map(function ($rows, $wid) {
                        $first    = $rows->first();
                        $districtName = trim((string) (isset($first->district_name) ? $first->district_name : ''));
                        $provinceName = trim((string) (isset($first->province_name) ? $first->province_name : ''));
                ...
                $result = $stores
                    ...
                    ->map(function ($rows, $wid) {
                        $first    = $rows->first();
                        $wardName = trim((string) (isset($first->ward_name) ? $first->ward_name : ''));
                        $districtName = trim((string) (isset($first->district_name) ? $first->district_name : ''));
                        $provinceName = trim((string) (isset($first->province_name) ? $first->province_name : ''));
                        $items = $rows->map(function ($r) {
                            return [
                    ...
                    ->sortBy('ward_name', SORT_NATURAL | SORT_FLAG_CASE)
                    ->values()
                    ->all();
            }


            return view('site.store_detail', compact('result', 'partners', 'distSlug', 'proSlug', 'banner'));
        }
    ...
    public function blogDetail(Request $request, $slug) {
        ...
        return view('site.blog_detail', compact('blog', 'othersBlog', 'categories', 'banners'));
    }
    ...
    public function postContactQuick(Request $request)
    {
        $rule  =  [
            'name'  => 'required',
            'title'  => 'required',
    ...
    public function submitRating(Request $request) {
        $rule  =  [
            ...
            'formReview.content_text' => 'required|string|min:5|max:2000',
        ];
        ...
        $validate = Validator::make(
            ...
            [
                ...
                'formReview.phone.regex' => 'Số điện thoại không đúng định dạng',
                'formReview.content_text.required' => 'Vui lòng nhập nội dung',
                'formReview.content_text.min'      => 'Nội dung quá ngắn (tối thiểu :min ký tự).',
                'formReview.content_text.max'      => 'Nội dung quá dài (tối đa :max ký tự).',
            ]
        );
        ...
        $normalized = preg_replace('/\s+/u', ' ', trim($contentText));
        if (mb_strlen($normalized, 'UTF-8') > 2000) {
            return response()->json([
                'errors' => ['formReview.content_text' => ['Nội dung quá dài (tối đa 2000 ký tự).']]
            ], 422);
        }

        if (strlen($contentHtml) > 20000) {
            return response()->json([
                'errors' => ['formReview.content_html' => ['Nội dung có quá nhiều định dạng, vui lòng rút gọn.']]
            ], 422);
        }
        ...
        $review->content = $data['content_html'];
